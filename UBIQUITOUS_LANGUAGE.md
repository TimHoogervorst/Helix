...
| **Table Kit** | The Shell-owned deep grid module (`src/shell/src/shared/table/`) exposed through `TableKit`. It accepts shape-resolved columns and controlled typed rows, and owns cell rendering/editing, selection, keyboard interaction, typed clipboard conversion, read-only filtering, bounds clamping, and grid layout. Table blocks resolve Column Types, map domain row objects to and from the grid, and retain persisted state, registration, formula previews, server calls, and block chrome; leading and trailing slots host block-owned status and action decorations | table framework |
...
| **Result Schema** | A Schema belonging to a `ResultTable`-tagged Schema Type. Like an entity Schema but with an Entity Column instead of the implicit Name Column. Managed in the Result Schemas tab of schema settings | result definition |
...
| **Result Table** | A block in an Entry's Rich-Text Document that loads a Result Schema and registers rows as Result Entities. It owns schema loading, formula evaluation and preview, registration, status, and server actions while TableKit owns its shape-resolved typed grid, controlled row mapping, interaction, clipboard behavior, and decoration slots. Like a Registry Table it has typed cells and an explicit register button, but each row inserts a source Entity into the Entity Column instead of typing a Name | results table, assay table |
