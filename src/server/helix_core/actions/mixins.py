...
def logs_action(action_type: str, **config: Any) -> Callable:
    ...
    Validates *action_type* against the registry at decoration time.
    ...
    Raises:
        ValueError: If *action_type* is a non-core action that has not
...
def _validate_action_type(action_type: str) -> None:
    """Validate *action_type* against the registry, if fully initialized.

    Core verbs always pass.  For other action types, checks that the
    owning mod (derived from the action_type prefix) has registered an
    action model.  When custom actions are registered for the mod, also
    validates that the specific action_type appears in the catalog.
    ...
    # Derive the mod from the action_type prefix (first dot-segment).
    ...
    if not registry.has_action_model(mod_id):
        raise ValueError(
            f"Unknown action type '{action_type}'. The mod '{mod_id}' "
    ...
    # specific action_type appears in the catalog.  If the mod hasn't
    # registered any custom actions yet, we allow any action_type —
    # it will be validated at request time by log_action().
    catalog = registry.get_action_catalog(mod_id)
    has_custom = any(
        entry["core"] != entry["action_type"] for entry in catalog
    )
    if has_custom and not registry.validate_action(action_type):
        raise ValueError(
            f"Unknown action type '{action_type}'. Custom actions must be "
