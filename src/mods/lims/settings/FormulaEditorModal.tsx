...
} from "../../../shell/src/shared/hooks/useComputedFields";
...
export default function FormulaEditorModal({
  ...
  const references = useMemo(() => referencesIn(expression), [expression]);
  ...
  const validation = useMemo(() => {
    ...
    const formulas = Object.fromEntries(
      siblingColumns
        .filter(
          (item) =>
            item.type === "formula" && item.name.trim() && item.expression,
        )
    ...
    const invalid = Object.entries(results).find(([name, result]) => {
      ...
      const formulaCalls = functionCallsIn(formulas[name]?.expression ?? "");
      return !formulaCalls.every((functionName) =>
        catalogIds.has(functionName),
      );
    });
  ...
  const evaluate = async () => {
    ...
    try {
      ...
      if (!usesBackendOnlyFunction(expression)) {
        setEvaluation(evaluateFormula(expression, row));
        return;
      }
      ...
      setEvaluation(response.results[column.name]);
    } catch (error) {
      setEvaluation({
        ok: false,
        error: {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`Formula Editor: ${column.name || "New field"}`}
      className="max-w-2xl"
    >
      <div className="space-y-4">
        <div className="relative">
          <label
            className="mb-1 block text-xs font-medium text-[var(--color-ink-muted-foreground)]"
          ...
          {showSuggestions && (
            <div
              className="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-auto rounded-md border border-[var(--color-ink-hairline)] bg-[var(--color-card)] p-1 shadow-lg"
            ...
            >
              {suggestions.map((suggestion) => (
                ...
                >
                  ...
                  <span className="ml-2 text-[var(--color-ink-muted-foreground)]">
        ...
        <section className="rounded-md border border-[var(--color-ink-hairline)] p-3">
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            {references.map((name) => (
              <label
                key={name}
                className="text-xs text-[var(--color-ink-muted-foreground)]"
              >
                {name}
                <Input
                  value={samples[name] ?? ""}
          ...
          <Button
            ...
            className="mt-3"
            onClick={evaluate}
            disabled={!expression.trim() || evaluating}
          >
          ...
          {evaluation && (
            <p className="mt-3 text-sm" data-testid="formula-evaluation-result">
              {evaluation.ok
                ...
                : `${evaluation.error.code}: ${evaluation.error.message}`}
            </p>
          )}
        </section>
        ...
        <div className="flex justify-end gap-2 border-t border-[var(--color-ink-hairline)] pt-3">
          ...
          <Button
            type="button"
            onClick={() => onSave(expression, resultType)}
            disabled={Boolean(validation) || !expression.trim()}
          >
            Save expression
          </Button>
        </div>
